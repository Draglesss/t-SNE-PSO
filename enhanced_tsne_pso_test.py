...
def run_comparison(dataset_name, X, y, perplexity=30.0, n_iter=500):
    ...
    print("\nResults:")
    print(
        f"TSNE-PSO KL divergence: {tsne_pso.kl_divergence_:.6f} | Time: {pso_time:.2f}s"
    )
    if hasattr(tsne_pso, "convergence_history_"):
        print(f"TSNE-PSO convergence points: {len(tsne_pso.convergence_history_)}")
    print(
        f"Standard t-SNE KL divergence: {tsne_std.kl_divergence_:.6f} | Time: {std_time:.2f}s"
    )
    ...
    scatter0 = axes[0].scatter(
        embedding_pso[:, 0], embedding_pso[:, 1], c=y, cmap="viridis", alpha=0.8, s=40
    )
    ...
    scatter1 = axes[1].scatter(
        embedding_std[:, 0], embedding_std[:, 1], c=y, cmap="viridis", alpha=0.8, s=40
    )
    axes[1].set_title(
        f"Standard t-SNE | {dataset_name} | KL={tsne_std.kl_divergence_:.4f}"
    )
    ...
    plt.tight_layout()
    plt.savefig(
        f"{dataset_name.lower().replace(' ', '_')}_comparison.png",
        dpi=150,
        bbox_inches="tight",
    )
    plt.show()
    ...
    if hasattr(tsne_pso, "convergence_history_"):
        ...
        plt.tight_layout()
        plt.savefig(
            f"{dataset_name.lower().replace(' ', '_')}_convergence.png",
            dpi=150,
            bbox_inches="tight",
        )
        plt.show()
...
def main():
    ...
    for name, dataset in datasets.items():
        if name != "Digits":
            ...
            all_results[name] = run_comparison(
                name, X, y, perplexity=min(30.0, X.shape[0] / 5), n_iter=500
            )
    ...
    if "Digits" in datasets:
        ...
        all_results["Digits"] = run_comparison(
            "Digits (subsampled)",
            X_sub,
            y_sub,
    ...
    for name, result in all_results.items():
        ...
        print(
            f"{name:<20} {pso_kl:<15.6f} {std_kl:<15.6f} {improvement:<15.2f}% {pso_time:<15.2f}s {std_time:<15.2f}s"
        )
